...
class QKVDynaLinear(nn.Linear):
    ...
    def forward(self, input):
        ...
        if self.dyna_dim[1]:
            self.out_features_part = head_round_to_nearest(self.out_features_max, self.width_mult, self.num_heads)
        weight = torch.cat((
            self.weight[:self.out_features_part, :self.in_features], 
            self.weight[self.out_features_max:self.out_features_max+self.out_features_part, :self.in_features],
            self.weight[2*self.out_features_max:2*self.out_features_max+self.out_features_part, :self.in_features]
        ), axis=0
        )
        if self.bias is not None:
            bias = torch.cat((
                self.bias[:self.out_features_part],
                self.bias[self.out_features_max:self.out_features_max+self.out_features_part],
                self.bias[2*self.out_features_max:2*self.out_features_max+self.out_features_part]
            ), axis=0
            )
        else:
            bias = self.bias
...
class Mlp(nn.Module):
    ...
    """
    def __init__(self, in_features, num_heads, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = DynaLinear(in_features, hidden_features, num_heads=num_heads, dyna_dim=[False, True])
        self.act = act_layer()
        self.fc2 = DynaLinear(hidden_features, out_features, num_heads=num_heads, dyna_dim=[True, False])
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = QKVDynaLinear(dim, dim * 3, num_heads=num_heads, dyna_dim=[False, True], bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = DynaLinear(dim, dim, num_heads=num_heads, dyna_dim=[True, False])
    ...
    def forward(self, x):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C * num_heads // self.num_heads)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    default_cfg['source'] = "file"
    default_cfg['file'] = "C:/Users/MJ/Desktop/DynaViT/models/S_16-i21k-300ep-lr_0.001-aug_light1-wd_0.03-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.03-res_224.npz"
    default_cfg['custom_load'] = True
    default_cfg['num_classes']=200
    print(default_cfg)
    ...
    if 'pretrained_cfg' in kwargs:
        print("Warning: 'pretrained_cfg' found in kwargs. It will be ignored.")
        kwargs.pop('pretrained_cfg')
    model = build_model_with_cfg(
        VisionTransformer, variant, pretrained,
        pretrained_cfg=dict(default_cfg),
