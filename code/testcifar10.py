# python ./code/testcifar10.py --num_classes 10 --batch_size 4096 --device "cuda:1" --img_size 32 
...
save_dir = str(increment_path(Path(ROOT / "output" / "test" /'0822cifar10-prun'), exist_ok=False))
...
val_dataset = CIFAR10(root = './', train = False, transform=val_transforms)
...
for i, width in enumerate(tqdm([0.25, 0.5, 0.75, 1], desc="Width", leave=False)):
    ...
    for j, depth in enumerate(tqdm([0.25, 0.5, 0.75, 1], desc="Depth", leave=False)):
        path = os.path.join("code/testmodelcifar10", f"Width{width}_model_width_distillation.pt")
        # path = os.path.join("code/testmodelcifar10", f"Width{width}_Depth{depth}_model_width_distillation.pt")
        try:
...
# path = os.path.join("/home/jxt/docworkspace/ViT_pruning/code/testmodelcifar10/vit-small-224-cifar10-finetuned-1.0.pth")
# logger.info(path)
# model.load_state_dict(torch.load(path,weights_only=True), strict=False)
