...
def head_round_to_nearest(input_size, width_mult, num_heads, min_value=1):
    new_num_heads = max(min_value, round(num_heads * width_mult))
    assert input_size % num_heads == 0, "Dim should be a multiple of number of heads"
    head_dim = input_size // num_heads
    new_input_size = head_dim * new_num_heads
    return new_input_size
...
class QKVDynaLinear(nn.Linear):
    def __init__(
        ...
        super(QKVDynaLinear, self).__init__(in_features, out_features, bias=bias)
    ...
    def forward(self, input):
        ...
        if self.dyna_dim[1]:
            self.out_features_part = head_round_to_nearest(
                self.out_features_max, self.width_mult, self.num_heads
            )
        weight = torch.cat(
            (
                self.weight[: self.out_features_part, : self.in_features],
                self.weight[
                    self.out_features_max : self.out_features_max
                    + self.out_features_part,
                    : self.in_features,
                ],
                self.weight[
                    2 * self.out_features_max : 2 * self.out_features_max
                    + self.out_features_part,
                    : self.in_features,
                ],
            ),
        ...
        if self.bias is not None:
            bias = torch.cat(
                (
                    self.bias[: self.out_features_part],
                    self.bias[
                        self.out_features_max : self.out_features_max
                        + self.out_features_part
                    ],
...
class Attention(nn.Module):
    def __init__(
        ...
        #     dim, dim, num_heads=num_heads, dyna_dim=[True, False], width_mult=width_mult
        # )
        num_heads_prune = round(self.num_heads * self.qkv.width_mult)
        self.projs = nn.ModuleList(
            [nn.Linear(head_dim, dim, bias=False) for _ in range(num_heads_prune)]
        )
        self.proj_bias = nn.parameter.Parameter(torch.zeros(dim))
        self.proj_drop = nn.Dropout(proj_drop)
        if not self.no_ghost and num_heads_prune < num_heads:
            if ghost_mode == "dense":
                self.ghost_modules = nn.ModuleList(
                    [
                        nn.Conv2d(
                            dim,
                            dim,
                            (ghost_kernel_size, num_heads_prune),
                            1,
                            (ghost_kernel_size // 2, 0),
                            groups=dim,
                        )
                        for _ in range(self.num_heads - num_heads_prune)
                    ]
                )
            else:
                self.ghost_module = nn.Conv2d(
                    num_heads_prune,
                    ((self.num_heads - num_heads_prune - 1) // num_heads_prune + 1)
                    * num_heads_prune,
                    ghost_kernel_size,
                    1,
                    ghost_kernel_size // 2,
                    groups=num_heads_prune,
                )
    ...
    def reorder_heads(self, idx):
        ...
        reorder_head_matrix(self.qkv, index_qkv)
        proj_tensors = [x.weight.clone().detach() for x in self.projs]
        assert len(proj_tensors) == self.num_heads, "Reordering only allowed in full width"
        proj_tensors = [proj_tensors[i] for i in idx]
        for i in range(len(self.projs)):
            self.projs[i].weight.requires_grad = False
    ...
    def forward(self, x, head_mask=None):
        ...
        qk = self.qkv(x)
        # print(qk.shape)
        qkv = qk.reshape(B, N, 3, num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = (
            qkv[0],
            qkv[1],
            qkv[2],
        ...
        x = torch.stack([proj(x[:, :, i]) for i, proj in enumerate(self.projs[:num_heads])])
        if self.no_ghost:
            x = torch.sum(x, dim=0)
        elif num_heads < self.num_heads:
            if self.ghost_mode == "dense":
                x_ghost_input = x.reshape(B, -1, N, x.shape[0])
                x_ghosts = torch.stack(
                    [ghost(x_ghost_input) for ghost in self.ghost_modules]
                )
                x_ghosts = x_ghosts[:, :, :, :, 0].transpose(-2, -1)
                x = torch.sum(x, dim=0) + torch.sum(x_ghosts, dim=0)
            else:
                x_ghost_input = x.reshape(B, x.shape[0], N, -1)
                x_ghosts = self.ghost_module(x_ghost_input).transpose(0, 1)
                x_ghosts = x_ghosts[: self.num_heads - num_heads]
                x = torch.sum(x, dim=0) + torch.sum(x_ghosts, dim=0)
        else:
            x = torch.sum(x, dim=0)
        x = x + self.proj_bias
